...
The hosted Qwen 3.8 FP8 rows used the official `Qwen/Qwen3.8-27B-FP8` checkpoint on a dedicated Baseten single-H100 SGLang deployment at a 262,144-token native context (BF16 KV cache, no speculative decoding, `reasoning.effort` low or xhigh, model-card thinking sampling, no output-token cap). Each row pools two independent 25-run cohorts (the 2026-08-16 production campaign and a 2026-08-18 extension on the same deployment); the raw file sets are ledger-derived from the campaign `canonical.tsv` files rather than a filename glob. The pooled n=50 medians are 90 (low, 50/50 complete) and 92.5 (xhigh, 46/50) — the even run count makes 92.5 the board's first fractional median. The xhigh cohorts independently reproduced a 2-in-25 convergence-failure rate; the first 25-run xhigh median read 96 before pooling, a reminder that a single 25-run median on this score distribution can move several points on replication.
...
| Qwen/Qwen3.8-27B-FP8 (th=xhigh, base=model-wom2d5kq.api.baseten.co/deployment/wnoy9p0/sync) | 50 | 92.5 | 92.0% | 9.6 | 13.8 | 14.2 | 13.8 | 2047.8 | 12771.2 | 227.13 |
...
| Qwen/Qwen3.8-27B-FP8 (th=low, base=model-wom2d5kq.api.baseten.co/deployment/wnoy9p0/sync) | 50 | 90 | 100.0% | 7.0 | 14.8 | 12.9 | 15.0 | 2077.7 | 10922.7 | 210.11 |
