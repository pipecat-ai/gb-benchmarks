...
echo "SOAK_CONFIG deployment=sglang mode=high runs=5 temperature=1.0 top_p=0.95 max_tokens=omitted thinking_budget=omitted pipeline_idle_timeout_secs=900"

for run_number in $(seq -w 1 5); do
  round_id="soak${run_number}"
  stem="nemotron-3.5-lightning-natural-high-unbounded-sglang-soak-${SOAK_STAMP}-r${run_number}"
