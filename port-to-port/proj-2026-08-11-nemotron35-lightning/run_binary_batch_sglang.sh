...
echo "BATCH_CONFIG deployment=sglang modes=none,high runs_per_mode=25 temperature=1.0 top_p=0.95 max_tokens=omitted thinking_budget=omitted pipeline_idle_timeout_secs=900"
...
for mode in none high; do
  ...
  for run_number in $(seq -w 1 25); do
    ...
    stem="nemotron-3.5-lightning-natural-${mode_label}-sglang-${BATCH_STAMP}-${round_id}"
